...
class SuppliersController extends AppController
{
	...
	public function AddSupplierPic() {
		if ($this->request->is('ajax')) {
			$this->autoRender = false;
			$SupplierPic = TableRegistry::get('Supplier_pics');
			$supplier_pic = $SupplierPic->newEntity();	
				$supplier_pic = $SupplierPic->patchEntity($supplier_pic, $this->request->data);
				if ($SupplierPic->save($supplier_pic)) {
					$supplier_pics = $SupplierPic->find()->where(['supplier_id'=>$this->request->data['supplier_id']]);
					$html = '';
					foreach ($supplier_pics as $key => $supplier_pic) {
						$html .= '<option value="'.$supplier_pic->id.'">'.$supplier_pic->name.'</option>';
					}
					$msg = array('status' => true, 'html' => $html);
				} else {
	...
	public function SelectSupplierPic()	{
		if ($this->request->is('ajax')) {
			$this->autoRender = false;
			$SupplierPic = TableRegistry::get('Supplier_pics');
			$supplier_pics = $SupplierPic->find()->where(['supplier_id'=>$this->request->data['id']]);
			$html = '';
			foreach ($supplier_pics as $key => $supplier_pic) {
				$html .= '<option value="'.$supplier_pic->id.'">'.$supplier_pic->name.'</option>';
			}
